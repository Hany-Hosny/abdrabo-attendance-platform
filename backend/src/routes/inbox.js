...
inboxRouter.param("studentId", async (req, res, next, value) => {
  try {
    ...
    req.studentAccessStatus = req.studentAuthStatus;
    next();
...
async function studentIdFromRequest(req) { const student = await authenticatedStudent(req); req.studentAccessStatus = req.studentAuthStatus; return student?.id || null; }
function studentFailure(req, res, extra = {}) { return studentAuthFailure(req, res, extra); }
...
async function getThread(threadId, studentId = null) {
  const result = await query(`SELECT it.*, s.full_name, s.student_serial, s.student_code, s.is_active AS student_is_active, s.deleted_at AS student_deleted_at,
    COALESCE(g.display_name,g.name) AS group_name, COALESCE(g.grade_level,g.grade) AS grade_level,
    EXISTS (SELECT 1 FROM inbox_messages origin WHERE origin.thread_id=it.id AND origin.sender_type='student') AS has_student_message,
    (SELECT COUNT(*) FROM inbox_messages im WHERE im.thread_id=it.id AND im.deleted_at IS NULL AND im.is_read=FALSE AND im.sender_type IN ('student','public')) AS unread_count
...
export function serializeInboxThread(thread) {
  const linkedStudent = Boolean(thread.student_id || thread.has_student_message);
  const replyAllowed = isEligibleInboxReplyRecipient(thread);
  return {
    ...thread,
    source_type: linkedStudent ? "registered_student" : "public_inquiry",
    reply_allowed: replyAllowed,
    read_status: Number(thread.unread_count || 0) > 0 ? "unread" : "read",
    studentId: linkedStudent ? thread.student_id : null,
    student_id: linkedStudent ? thread.student_id : null,
    full_name: linkedStudent ? thread.full_name : null,
    student_code: linkedStudent ? thread.student_code : null,
    student_serial: linkedStudent ? thread.student_serial : null,
    group_name: linkedStudent ? thread.group_name : null,
    grade_level: linkedStudent ? thread.grade_level : null,
    public_name: linkedStudent ? null : thread.public_name,
    public_phone: linkedStudent ? null : thread.public_phone
  };
}

export function isEligibleInboxReplyRecipient(thread) {
  return Boolean(thread?.student_id && thread.student_is_active && !thread.student_deleted_at);
}

async function markIncomingRead(threadId, senderTypes) {
  return query("UPDATE inbox_messages SET is_read=TRUE WHERE thread_id=$1 AND deleted_at IS NULL AND is_read=FALSE AND sender_type = ANY($2::text[]) RETURNING id", [threadId, senderTypes]);
}
...
inboxRouter.get("/student/:studentId/inbox", async (req, res, next) => {
  try {
    if (!req.studentAccess) return studentFailure({ ...req, studentAuthStatus: req.studentAccessStatus }, res);
...
inboxRouter.get("/student/inbox", async (req,res,next)=>{try{const studentId=await studentIdFromRequest(req);if(!studentId)return studentFailure(req,res);const threads=await query(`SELECT it.*,(SELECT COUNT(*) FROM inbox_messages im WHERE im.thread_id=it.id AND im.deleted_at IS NULL AND im.is_read=FALSE AND im.sender_type IN ('admin','teacher','assistant'))::int AS unread_count,(SELECT body FROM inbox_messages WHERE thread_id=it.id AND deleted_at IS NULL ORDER BY created_at DESC LIMIT 1) AS last_message FROM inbox_threads it WHERE it.student_id=$1 ORDER BY it.updated_at DESC`,[studentId]);res.json({ok:true,threads:threads.rows,unread_count:threads.rows.reduce((sum,row)=>sum+Number(row.unread_count||0),0)});}catch(error){next(error);}});

inboxRouter.get("/student/:studentId/inbox/unread-count", async (req,res,next)=>{ try { if(!req.studentAccess)return studentFailure({ ...req, studentAuthStatus: req.studentAccessStatus },res); const result=await query("SELECT COUNT(*)::int AS count FROM inbox_messages im JOIN inbox_threads it ON it.id=im.thread_id WHERE it.student_id=$1 AND im.is_read=FALSE AND im.sender_type <> 'student'",[Number(req.params.studentId)]); res.json({ok:true,count:result.rows[0].count}); } catch(error){next(error);} });

inboxRouter.get("/student/:studentId/inbox/:threadId/messages", async (req,res,next)=>{ try { if(!req.studentAccess)return studentFailure({ ...req, studentAuthStatus: req.studentAccessStatus },res); const thread=await getThread(req.params.threadId,Number(req.params.studentId)); if(!thread)return res.status(404).json({ok:false,status:"not_found"}); const messages=await getMessages(req.params.threadId); res.json({ok:true,thread,messages:messages.rows}); }catch(error){next(error);} });

inboxRouter.post("/student/:studentId/inbox", async (req,res,next)=>{ try { if(!req.studentAccess)return studentFailure({ ...req, studentAuthStatus: req.studentAccessStatus },res); const studentId=Number(req.params.studentId), subject=clean(req.body?.subject), body=clean(req.body?.body); if(!studentId||!subject||!body)return res.status(400).json({ok:false,status:"invalid_message"}); const thread=await query("INSERT INTO inbox_threads(student_id,subject) VALUES($1,$2) RETURNING *",[studentId,subject]); const message=await addMessage(thread.rows[0].id,"student",studentId,body,req); res.status(201).json({ok:true,thread:thread.rows[0],message}); }catch(error){next(error);} });
inboxRouter.post("/student/inbox/messages", async (req,res,next)=>{try{const studentId=await studentIdFromRequest(req),subject=clean(req.body?.subject),body=clean(req.body?.body);if(!studentId)return studentFailure(req,res);if(!subject||!body)return res.status(400).json({ok:false,status:"invalid_message"});const thread=await query("INSERT INTO inbox_threads(student_id,subject) VALUES($1,$2) RETURNING *",[studentId,subject]);const message=await addMessage(thread.rows[0].id,"student",studentId,body,req);res.status(201).json({ok:true,thread:thread.rows[0],message});}catch(error){next(error);}});

inboxRouter.post("/student/:studentId/inbox/:threadId/messages", async (req,res,next)=>{ try { if(!req.studentAccess)return studentFailure({ ...req, studentAuthStatus: req.studentAccessStatus },res); const body=clean(req.body?.body), thread=await getThread(req.params.threadId,Number(req.params.studentId)); if(!thread)return res.status(404).json({ok:false,status:"not_found"}); if(!body)return res.status(400).json({ok:false,status:"invalid_message"}); const message=await addMessage(thread.id,"student",Number(req.params.studentId),body,req); res.status(201).json({ok:true,message}); }catch(error){next(error);} });
inboxRouter.put("/student/inbox/:threadId/read", async(req,res,next)=>{try{const studentId=await studentIdFromRequest(req);if(!studentId)return studentFailure(req,res);const thread=await getThread(req.params.threadId,studentId);if(!thread)return res.status(404).json({ok:false,status:"not_found"});const result=await query("UPDATE inbox_messages SET is_read=TRUE WHERE thread_id=$1 AND deleted_at IS NULL AND is_read=FALSE AND sender_type IN ('admin','teacher','assistant') RETURNING id",[req.params.threadId]);if(result.rowCount)await auditLog({action:"message_read_status_changed",studentId,details:{thread_id:Number(req.params.threadId),marked_count:result.rowCount,status_after:"read"},request:req});res.json({ok:true,marked_count:result.rowCount});}catch(error){next(error);}});
...
async function markStaffRead(req,res,next){try{const result=await markIncomingRead(req.params.id,["student","public"]);if(result.rowCount)await auditLog({action:"message_read_status_changed",actorId:req.teacher.id,details:{thread_id:Number(req.params.id),marked_count:result.rowCount,status_after:"read"},request:req});res.json({ok:true,marked_count:result.rowCount});}catch(error){next(error);}}
...
async function replyToThread(req,res,next){try{const body=clean(req.body?.body),thread=await getThread(req.params.id);if(!thread)return res.status(404).json({ok:false,status:"not_found"});if(!isEligibleInboxReplyRecipient(thread))return res.status(403).json({ok:false,status:"public_inquiry_reply_not_allowed"});if(!body)return res.status(400).json({ok:false,status:"invalid_message"});const message=await addMessage(thread.id,senderType(req.teacher.role),req.teacher.id,body,req);res.status(201).json({ok:true,message});}catch(error){next(error);}}
staffInboxRouter.post("/inbox/threads/:id/messages", requirePermission("messages.manage"), replyToThread);
staffInboxRouter.post("/inbox/:id/messages", requirePermission("messages.manage"), replyToThread);
