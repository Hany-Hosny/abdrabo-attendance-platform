...
function CollectionPanel({ collection, language, t, period }: { collection: DashboardData["collection"]; language: Language; t: DashboardTranslator; period: string }) {
  ...
  return <section className="dashboard-panel collection-panel">
    <div className="panel-heading"><div><span className="panel-kicker">{t("dashboard.collectionStatus")}</span><h2>{t("dashboard.collectionStatus")} <small>— {periodLabel(period, t)}</small></h2></div><span className="panel-icon">◔</span></div>
...
export function AdminExecutiveDashboard({ token, language, t, can, onNavigate }: Props) {
